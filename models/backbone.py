...
from models.base import IBackbone
...
class Backbone2GGCN(IBackbone):
    ...
    def forward(
        ...
        Returns:
            z:              (B, N, D)   — Z pt HOLIC MI loss (primul nivel BiRNN)
        ...
        # Z pentru HOLIC (primul nivel BiRNN, dupa fusion geometric)
        ...
        return {
            ...
            "segment_logits_bms": seg_logits_4d,          # (B, M, S, C) for temporal consistency
            "u_soft":         bms_to_bn(u_soft_bms),      # (B, N)
            "u_soft_bms":     u_soft_bms,                 # (B, M, S) for temporal consistency
            "u_hard":         bms_to_bn(u_hard_bms),      # (B, N)
            "u_hard_bms":     u_hard_bms,                 # (B, M, S) for temporal consistency
        }

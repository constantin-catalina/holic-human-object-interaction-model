"""
losses.py
Loss-urile HOLIC conform paperurilor ASSIGN + HOLIC:

  L_total = L_Label + L_Ant + lambda1*L_Seg + lambda2*L_MI + lambda3*L_Cos  (HOLIC Eq. 4)
...
class BoundaryUtils:
    """
    Clasă utilitară fără stare — conține doar metode statice pentru
    construirea ground-truth-ului de granită și smoothing Gaussian.
    """
    ...
    def compute_boundary_gt(
        ...
        boundary_gt_smooth = BoundaryUtils._gaussian_smooth_1d(boundary_gt, sigma)
...
class SegmentationLoss(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            boundary_smooth = BoundaryUtils.compute_boundary_gt(frame_labels, self.sigma)
...
# ---------------------------------------------------------------------------
# Temporal Consistency Loss (C2)
# ---------------------------------------------------------------------------

def temporal_consistency_loss(
    segment_logits_bms: torch.Tensor,
    u_soft_bms: torch.Tensor,
) -> torch.Tensor:
    """
    Penalize label disagreement between adjacent frames within segments.

    Uses MSE between adjacent softmax distributions (standard in action
    segmentation, e.g. MS-TCN). MSE directly measures how much probability
    mass shifts between frames and is stronger than KL when distributions
    are already smooth from the BiRNN.

        L_TC = sum_{t} (1 - u_soft_t) * MSE(softmax(z_t), softmax(z_t+1))

    Args:
        segment_logits_bms: (B, M, S, C) unflattened segment logits
        u_soft_bms:         (B, M, S) soft boundary probability (1 = boundary, 0 = inside segment)

    Returns:
        Scalar loss (weighted mean MSE).
    """
    assert segment_logits_bms.dim() == 4, f"Expected 4D logits, got {segment_logits_bms.dim()}D"
    assert u_soft_bms.dim() == 3, f"Expected 3D u_soft, got {u_soft_bms.dim()}D"

    probs = F.softmax(segment_logits_bms, dim=-1)   # (B, M, S, C)

    p_t  = probs[:, :, :-1, :]   # (B, M, S-1, C)
    p_t1 = probs[:, :, 1:,  :]   # (B, M, S-1, C)

    mse = ((p_t - p_t1) ** 2).sum(-1)   # (B, M, S-1)

    # Soft mask: high weight inside segments (u_soft ≈ 0), low weight at boundaries (u_soft ≈ 1)
    weight = (1.0 - u_soft_bms[:, :, :-1]).clamp(min=0.0)   # (B, M, S-1)
    return (mse * weight).sum() / (weight.sum() + 1e-8)
...
class HOLICLoss(nn.Module):
    """
    Loss total HOLIC (Eq. 4 din paper) + C6b prompt regularization + C2 temporal consistency:

        L = L_Label + lambda_ant*L_Ant + lambda1*L_Seg
              + lambda2*L_MI + lambda3*L_Cos
              + lambda4*L_PromptReg   (C6b only; 0 otherwise)
              + lambda_tc*L_TC        (C2 only; 0 otherwise)

    L_TC = MSE between adjacent frame softmax distributions, masked by (1 - u_soft).
    ...
    Args:
        ...
        lambda_tc:   coeficient L_TC (default=0.0, C2 temporal consistency)
    ...
    def forward(
        ...
        segment_logits_bms:  Optional[torch.Tensor] = None,  # C2
        u_soft_bms:          Optional[torch.Tensor] = None,  # C2
    ) -> dict:
        """
        Returns:
        ...
        # C2 temporal consistency loss
        if (
            self.lambda_tc > 0
            and segment_logits_bms is not None
            and u_soft_bms is not None
        ):
            l_tc = temporal_consistency_loss(segment_logits_bms, u_soft_bms)
        else:
            l_tc = torch.tensor(0.0, device=segment_logits.device)

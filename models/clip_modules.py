...
from models.base import IProjection, ITextEncoder
...
class MLPProjection(IProjection):
    """
...
class SimpleMLPProjection(IProjection):
    """
    Original HOLIC MLP: two linear layers with GELU, no dropout, no L2 norm.
...
class CLIPTextEncoder(ITextEncoder):
    """
    ...
    C6b additions:
      - learnable_temperature: a scalar log-temperature initialized to log(1/0.07)
        (same as CLIP's default), applied to cosine similarities in HOLIC.py.
        Exposed as self.log_temp so HOLIC.py can use it directly.
    ...
    def __init__(
        ...
        # Populated by CLIPTextEncoder.precompute_frozen_T() called from HOLIC.py.
